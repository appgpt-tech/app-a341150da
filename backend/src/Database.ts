//Source code generated by AppGPT (www.appgpt.tech)

//Class to create tables and seed new database
import { DataSource } from 'typeorm';
import { DBConfiguration } from './Configuration';
import { SettingsEntity } from './db/Settings.entity';
//autogenerate imports based on resources
import { usersEntity } from './db/users.entity';
import { departmentsEntity } from './db/departments.entity';
import { initiativesEntity } from './db/initiatives.entity';
import { goalsEntity } from './db/goals.entity';
import { emissionSourcesEntity } from './db/emissionSources.entity';
import { resourceUsageEntity } from './db/resourceUsage.entity';

export class Database {
  static dbConfiguration: DBConfiguration;
  public static ds: DataSource;

  static async Initialize(dbConfiguration: DBConfiguration) {
    Database.dbConfiguration = dbConfiguration;
    let dbConfig: any = dbConfiguration as any;
    //Autogenerate entities array from resource names

    dbConfig.entities = [
      SettingsEntity,
      usersEntity,
      departmentsEntity,
      initiativesEntity,
      goalsEntity,
      emissionSourcesEntity,
      resourceUsageEntity,
    ];
    Database.ds = new DataSource(dbConfig);
    await Database.ds.initialize();

    //TODO: Drop all tables

    await Database.Seed();
  }
  static async Seed() {
    let data: any = {
      users: [
        {
          userName: 'userName 1',
          department: 1,
          password: 'password 1',
          email: 'email 1',
          id: 23,
        },
        {
          userName: 'userName 2',
          department: 2,
          password: 'password 2',
          email: 'email 2',
          id: 16,
        },
        {
          userName: 'userName 3',
          department: 3,
          password: 'password 3',
          email: 'email 3',
          id: 40,
        },
        {
          userName: 'userName 4',
          department: 4,
          password: 'password 4',
          email: 'email 4',
          id: 87,
        },
        {
          userName: 'userName 5',
          department: 5,
          password: 'password 5',
          email: 'email 5',
          id: 43,
        },
      ],
      departments: [
        { departmentName: 'departmentName 1', departmentHead: 1, id: 34 },
        { departmentName: 'departmentName 2', departmentHead: 2, id: 77 },
        { departmentName: 'departmentName 3', departmentHead: 3, id: 89 },
        { departmentName: 'departmentName 4', departmentHead: 4, id: 47 },
        { departmentName: 'departmentName 5', departmentHead: 5, id: 26 },
      ],
      initiatives: [
        {
          name: 'name 1',
          description: 'description 1',
          startDate: '2023-05-11T01:46:51.842Z',
          endDate: '2024-09-14T03:14:42.394Z',
          relatedDepartment: 1,
          status: 'status 1',
          id: 46,
        },
        {
          name: 'name 2',
          description: 'description 2',
          startDate: '2023-10-10T17:29:10.097Z',
          endDate: '2024-02-24T21:56:11.218Z',
          relatedDepartment: 2,
          status: 'status 2',
          id: 83,
        },
        {
          name: 'name 3',
          description: 'description 3',
          startDate: '2024-02-21T13:28:23.709Z',
          endDate: '2024-08-01T04:11:58.606Z',
          relatedDepartment: 3,
          status: 'status 3',
          id: 97,
        },
        {
          name: 'name 4',
          description: 'description 4',
          startDate: '2023-06-11T17:46:59.410Z',
          endDate: '2024-09-05T05:23:39.363Z',
          relatedDepartment: 4,
          status: 'status 4',
          id: 54,
        },
        {
          name: 'name 5',
          description: 'description 5',
          startDate: '2023-09-02T16:57:05.809Z',
          endDate: '2023-06-29T07:24:12.318Z',
          relatedDepartment: 5,
          status: 'status 5',
          id: 91,
        },
      ],
      goals: [
        {
          name: 'name 1',
          description: 'description 1',
          targetDate: '2023-12-02T01:07:57.956Z',
          relatedDepartment: 1,
          status: 'status 1',
          id: 45,
        },
        {
          name: 'name 2',
          description: 'description 2',
          targetDate: '2025-01-06T12:01:15.302Z',
          relatedDepartment: 2,
          status: 'status 2',
          id: 7,
        },
        {
          name: 'name 3',
          description: 'description 3',
          targetDate: '2023-12-31T11:41:52.696Z',
          relatedDepartment: 3,
          status: 'status 3',
          id: 35,
        },
        {
          name: 'name 4',
          description: 'description 4',
          targetDate: '2024-07-07T00:39:45.074Z',
          relatedDepartment: 4,
          status: 'status 4',
          id: 91,
        },
        {
          name: 'name 5',
          description: 'description 5',
          targetDate: '2024-01-01T04:25:35.445Z',
          relatedDepartment: 5,
          status: 'status 5',
          id: 15,
        },
      ],
      emissionSources: [
        {
          sourceType: 'sourceType 1',
          quantityUsed: 0.57,
          emissionFactors: 0.34,
          totalEmissions: 0.72,
          id: 89,
        },
        {
          sourceType: 'sourceType 2',
          quantityUsed: 0.31,
          emissionFactors: 0.48,
          totalEmissions: 0.88,
          id: 76,
        },
        {
          sourceType: 'sourceType 3',
          quantityUsed: 0.5,
          emissionFactors: 0.76,
          totalEmissions: 0.11,
          id: 78,
        },
        {
          sourceType: 'sourceType 4',
          quantityUsed: 0.17,
          emissionFactors: 0.38,
          totalEmissions: 0.42,
          id: 50,
        },
        {
          sourceType: 'sourceType 5',
          quantityUsed: 0.94,
          emissionFactors: 0.52,
          totalEmissions: 0.53,
          id: 47,
        },
      ],
      resourceUsage: [
        {
          resourceType: 'resourceType 1',
          quantityConsumed: 0.68,
          unitOfMeasurement: 'unitOfMeasurement 1',
          periodOfConsumption: '2024-03-17T09:11:02.772Z',
          id: 72,
        },
        {
          resourceType: 'resourceType 2',
          quantityConsumed: 0.4,
          unitOfMeasurement: 'unitOfMeasurement 2',
          periodOfConsumption: '2024-12-04T19:06:23.308Z',
          id: 2,
        },
        {
          resourceType: 'resourceType 3',
          quantityConsumed: 0.4,
          unitOfMeasurement: 'unitOfMeasurement 3',
          periodOfConsumption: '2024-12-15T09:19:56.497Z',
          id: 50,
        },
        {
          resourceType: 'resourceType 4',
          quantityConsumed: 0.2,
          unitOfMeasurement: 'unitOfMeasurement 4',
          periodOfConsumption: '2024-11-03T14:43:19.918Z',
          id: 50,
        },
        {
          resourceType: 'resourceType 5',
          quantityConsumed: 0.94,
          unitOfMeasurement: 'unitOfMeasurement 5',
          periodOfConsumption: '2024-05-23T19:09:19.302Z',
          id: 89,
        },
      ],
    };
    //Autogenerate multiple such calls ie for each resource and its data object
    let isSeeded = await this.IsSeeded();
    //if (!isSeeded) {
    //forcing app recreation
    if (true) {
      console.log('   Seeding database...');
      await this.SeedResource('usersEntity', data.users);
      await this.SeedResource('departmentsEntity', data.departments);
      await this.SeedResource('initiativesEntity', data.initiatives);
      await this.SeedResource('goalsEntity', data.goals);
      await this.SeedResource('emissionSourcesEntity', data.emissionSources);
      await this.SeedResource('resourceUsageEntity', data.resourceUsage);
      await this.SeedResource('SettingsEntity', {
        settingname: 'isSeeded',
        settingvalue: 'true',
      });
    } else {
      console.log('   Database seeded already!');
    }
  }
  static async IsSeeded() {
    const repo = Database.ds.getRepository('SettingsEntity');
    let rec: any = await repo.findOne({
      select: {
        settingname: true,
        settingvalue: true,
      },
      where: {
        settingname: 'isSeeded',
      },
    });
    if (rec && rec.settingvalue) return true;
    return false;
  }
  static async SeedResource(resourceName: any, resourceData: any) {
    const repo = Database.ds.getRepository(resourceName);
    //await repo.clear();
    console.log('   Seeding table ' + resourceName);
    await repo.upsert(resourceData, ['id']);
  }
}
